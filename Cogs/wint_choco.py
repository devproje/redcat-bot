...
class WintChoco(commands.Cog):
    ...
    async def wint_choco(self, ctx, wincho_action):
        if wincho_action == "smash":
            ...
            await ctx.send(embed=embed)
        elif wincho_action == None:
            embed = discord.Embed(title=":stop_sign: Error!", descroption=f"You must type argument!", color=self.embed_color)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title=":stop_sign: Error!", descroption=f"{wincho_action} is not found!", color=self.embed_color)
            await ctx.send(embed=embed)
